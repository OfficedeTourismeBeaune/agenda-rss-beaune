...
DISCOVERY_PAGES = [
    "https://www.beaune-tourisme.fr/sejourner/agenda-semaine/",
    "https://www.beaune-tourisme.fr/sejourner/agenda-week-end/",
    "https://www.beaune-tourisme.fr/fr/sejourner/agenda/agenda-des-evenements-oenotouristiques/",
    "https://www.beaune-tourisme.fr/fr/profiter/festivals/agenda-des-festivals/",
    "https://www.beaune-tourisme.fr/fr/sejourner/agenda/",
]

# Six fiches de référence validées manuellement pour la semaine pilote.
# Elles servent seulement de "graines" de découverte : le filtre de date reste
# obligatoire, donc elles disparaîtront naturellement des semaines suivantes.
EDITORIAL_SEED_URLS = [
    "https://www.beaune-tourisme.fr/sejourner/agenda/concert-de-la-virade-de-lespoir-chagny-fr-6789236/",
    "https://www.beaune-tourisme.fr/sejourner/agenda/festival-dautomne-jazz-overre-beaune-3-jours-de-concerts-beaune-fr-4322650/",
    "https://www.beaune-tourisme.fr/sejourner/agenda/concert-de-tibz-loe-en-premiere-partie-beaune-fr-6727666/",
    "https://www.beaune-tourisme.fr/sejourner/agenda/hotel-dieu-hospices-de-beaune2026-mouvementsatelier-gustatif-saveurs-du-soin-beaune-fr-5326104/",
    "https://www.beaune-tourisme.fr/sejourner/agenda/livres-en-vignes-a-vougeot-vougeot-fr-4146278/",
    "https://www.beaune-tourisme.fr/sejourner/agenda/visite-guidee-des-remparts-beaune-fr-6552682/",
]
...
SESSION.headers.update({
    "User-Agent": "BeauneAgendaRSS/2.2 (+https://www.beaune-tourisme.fr/)"
})
...
REFERENCE_BOOSTS = {
    "virade de l'espoir": 15.0,
    "jazz o'verre": 15.0,
    "concert de tibz": 15.0,
    "saveurs du soin": 15.0,
    "livres en vignes": 15.0,
    "visite guidee des remparts": 15.0,
}
...
def discover_agenda_links() -> list[str]:
    """
    Découvre les fiches depuis plusieurs listes éditoriales du site au lieu
    de dépendre d'une seule playlist RSS / d'une seule page de résultats.

    On scanne aussi plusieurs pages paginées pour couvrir davantage de fiches.
    """
    links = []
    seen = set()
    ...
    def add(url: str):
        clean = clean_url(url)
        if valid_event_link(clean) and clean not in seen:
            seen.add(clean)
            links.append(clean)

    # Graines éditoriales validées pour la semaine pilote.
    for url in EDITORIAL_SEED_URLS:
        add(url)

    for base_url in DISCOVERY_PAGES:
        for page in range(1, AGENDA_PAGES + 1):
            page_url = base_url if page == 1 else (
                f"{base_url}{'&' if '?' in base_url else '?'}listpage={page}"
            )
            try:
...
def editorial_score(ev: Event, previous: set[str], rng: random.Random) -> float:
    ...
    for term, value in REFERENCE_BOOSTS.items():
